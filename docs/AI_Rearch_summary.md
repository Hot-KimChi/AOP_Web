...
| # | 요청 | 해결 | Detail |
|---|---|---|---|
| 15 | AOP Web 로그인을 Windows 인증으로 확인해 접속하도록 수정 | SQL 연결 계층에 `Trusted_Connection=yes` Windows 인증 모드를 추가했습니다. 단 SSMS의 서버 측 DB 인증만으로 브라우저 사용자의 Windows SSO가 되는 것은 아니므로, 웹 로그인·세션 연결은 SQL 인증으로 고정해 임의 사용자가 서버 계정 권한으로 로그인되는 보안 우회를 차단했습니다. AD/Kerberos 또는 Entra ID·IIS 구성이 선행되지 않은 현재 환경에서는 가짜 SSO를 구현하지 않았습니다. | [→ Detail](./AI_Rearch_detail.md#v0972--15-windows-인증과-웹-sso-경계-분리) |
